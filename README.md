...
**Flags:**
  ...
  **-c**, **--compress**
 Compress HTML files by stripping unnecessary spaces and empty lines
...
When those commands are expanded by ss2em, the source file should compile in the Arduino IDE.
...
Note, when creating a template you should double escape any escaped character inside a C/C++ string.
...
- Expansion directives (variables).
...
`[:::Compress:::]` | The execution argument for the flag  `--compress`
`[:::Use:::]` | The execution argument for the flag `--use`
`[:::Write:::]` | The execution argument for the flag  `--write`

## Site Files
A web site is composed of a series of files (HTML files, CSS files, JavaScript files, JPEG files, etc.), we call them site files.
ss2em process site files to be hosted by a micro-controller.

In general we expect site files to be generated by a static site generator.
However, it is important for the micro-controller to add dynamically generated data to those files.
There are at least two ways to achieve dynamic functionality on the HTML site files:

- Using Asynchronous JavaScript And XML (AJAX). You can embed JavaScript on the HTML site files.
The JavaScript code can request information from the micro-controller site and update the web page.
This can also be use to submit commands to the micro-controller from the web page.

- Embedding markers in the HTML site files.
ss2em parses HTML site files for `(:::<function-name>:::)` markers.
When markers are present, ss2em generates enough information for them to be processed in the template.
